...
class Booking extends BaseModel {
  ...
  async createBooking(data) {
    return await this.transaction(async (client) => {
      ...
      const table = await client.query('SELECT * FROM ban WHERE maban = $1', [maban]);
      if (table.rows.length === 0) {
        throw new Error('Không tìm thấy bàn');
      }

      if (table.rows[0].soghe < songuoi) {
        throw new Error(`Bàn chỉ có ${table.rows[0].soghe} ghế, không đủ cho ${songuoi} người`);
      }
      ...
      await client.query('UPDATE ban SET trangthai = $1 WHERE maban = $2', ['DaDat', maban]);
      ...
      `, [booking.rows[0].maphieu, `Đặt bàn mới: ${guest_hoten || 'Khách hàng'} - ${songuoi} người`]);
  ...
  async cancelBooking(identifier, manv = null, reason = null) {
    return await this.transaction(async (client) => {
      ...
      if (identifier.token && moment().isAfter(moment(bookingInfo.cancel_deadline))) {
