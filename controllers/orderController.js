...
const getAllOrders = async (req, res) => {
  try {
    ...
    const formattedOrders = orders.map(order => {
      ...
      return {
        ...order,
        monan_text: monanText.length > 100 ? monanText.substring(0, 100) + '...' : monanText
      };
    });

    res.json(formatResponse(true, formattedOrders, "Lấy danh sách đơn hàng thành công"));
  } catch (error) {
...
const getOrderById = async (req, res) => {
  try {
    ...
    if (!order) {
      return res.status(404).json(formatErrorResponse("Không tìm thấy đơn hàng"));
    }
...
const createOrder = async (req, res) => {
  try {
    ...
    if (!monAn || !Array.isArray(monAn) || monAn.length === 0) {
      return res.status(400).json(formatErrorResponse("Vui lòng chọn ít nhất một món"));
    }

    // Validate dữ liệu món ăn
    for (const item of monAn) {
      if (!item.id || !item.type || !item.soluong || item.soluong <= 0) {
        return res.status(400).json(formatErrorResponse("Dữ liệu món ăn không hợp lệ"));
      }
      if (!['monan', 'setbuffet'].includes(item.type)) {
        return res.status(400).json(formatErrorResponse("Loại món không hợp lệ"));
      }
    }

    const order = await Order.createOrder({ monAn, ghichu });
    res.status(201).json(formatResponse(true, order, "Tạo đơn hàng thành công"));
...
const confirmOrder = async (req, res) => {
  try {
    ...
    res.json(formatResponse(true, invoice, "Xác nhận đơn hàng thành công"));
